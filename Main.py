...
app.config['MYSQL_DATABASE'] = 'databse'  # MySQL database name
app.config['MYSQL_HOST'] = 'app-db-mysql-srv'  # Database host
mysql = MySQL(app)
...
def login():
    ...
    try:
        ...
        if user:
            ...
            conn.commit()
            return jsonify({'message': 'Login successful', 'user': {'user_id': user_id, 'name': name, 'role': role}}), 200
        else:
...
@app.route('/orders_for_dhl', methods=['GET'])
def get_orders_for_dhl():
    conn = mysql.connection
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT * FROM Orders WHERE courier = 'DHL'")
    orders = cursor.fetchall()
    if not orders:
        return jsonify({"message": "No orders found for DHL"}), 404
    return jsonify({"orders": orders}), 200
...
def reject_order(order_id):
    print("Reject Order endpoint called")
    conn = mysql.connection
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE Orders SET status = %s WHERE order_id = %s", ("rejected", order_id))
        conn.commit()
        if cursor.rowcount == 0:
            return jsonify({'message': 'Order not found or already rejected'}), 404
        return jsonify({'message': 'Order status updated to rejected'}), 200
    except Exception as e:
        print("Error updating order status:", e)
        return jsonify({'error': str(e)}), 500
    finally:
